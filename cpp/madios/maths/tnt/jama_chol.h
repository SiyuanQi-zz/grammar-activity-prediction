...
*/
template <class Real>
const Array2D<Real>& Cholesky<Real>::getL() const
{
	return L_;
}
...
*/
template <class Real>
Array1D<Real> Cholesky<Real>::solve(const Array1D<Real> &b) const
{
	int n = L_.dim1();
	assert (b.dim1() == n);
...
*/
template <class Real>
Array2D<Real> Cholesky<Real>::solve(const Array2D<Real> &B) const
{
	int n = L_.dim1();
	assert (B.dim1() == n);
